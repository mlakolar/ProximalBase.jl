@testset "sparseIterate" begin
    ...
    @testset "other" begin
        ...
        @test full(x) == Matrix(M)
        @test convert(Matrix, x) == Matrix(M)
        @test convert(Array, x) == Matrix(M)
    ...
    @testset "AtomIterate" begin
        ...
        @test length(x) == 6
        @test size(x) == (2,3)
        @test IndexStyle(typeof(x)) == IndexLinear() == true
        ...
        g = ProxL2(1., [1., 10.])
        out = prox(g, x)
        @test out.storage[1,:] == roughly((1. - 1. / norm(y[1,:]))*y[1,:])
        @test out.storage[2,:] == zeros(3)

        g = ProxL2(1., [1., 1.])
        out = prox(g, x)
        @test out.storage[1,:] == roughly((1. - 1. / norm(y[1,:]))*y[1,:])
        @test out.storage[2,:] == roughly((1. - 1. / norm(y[2,:]))*y[2,:])
